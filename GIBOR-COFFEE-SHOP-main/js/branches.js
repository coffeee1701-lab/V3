...
========================================================================================
                           ...
                           LOGIC DANH SÁCH & GIỚI THIỆU CHI NHÁNH
...
(function () {
  ...
  function renderBranchDetail(branch) {
    ...
    if (branch.contactPhone) {
      ...
      elements.detailPhone.href = "tel:" + branch.contactPhone.replace(/\s+/g, "");
    } else {
  ...
  function highlightActiveCard() {
    ...
    elements.branchGrid.querySelectorAll(".branch-card").forEach((card) => {
      card.classList.toggle("active", card.dataset.branchId === state.activeBranchId);
    });
  }
  ...
  function selectBranch(branchId, options) {
    const settings = { updateQuery: true, scrollToDetail: false, ...(options || {}) };
    ...
    if (settings.scrollToDetail && window.innerWidth <= 768 && elements.detailPanel) {
  ...
  function applyFilters() {
    ...
    state.filteredBranches = window.GIBOR_BRANCH_UTILS.search(keyword, cityCode);
    renderBranchCards(state.filteredBranches);

    if (elements.resultCount) {
      elements.resultCount.textContent = formatCountText(state.filteredBranches.length);
    }
    ...
    selectBranch(branchIdToSelect, { updateQuery: true, scrollToDetail: false });
  }
  ...
  function bindEvents() {
    ...
    if (elements.branchGrid) {
      elements.branchGrid.addEventListener("click", (event) => {
        ...
        if (!card) return;
        selectBranch(card.dataset.branchId, { updateQuery: true, scrollToDetail: true });
      });
    }
  }
  ...
  function initBranchesPage() {
    ...
    if (branchFromUrl) {
      ...
      applyFilters();
      selectBranch(branchFromUrl.id, { updateQuery: true, scrollToDetail: false });
    }
  }
